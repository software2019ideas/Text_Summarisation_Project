...
public class WordFrequencyCorpus {
	...
	public ArrayList<String> WordFrequency () {
		...
		Scanner scanner = new Scanner(new File("PUT YOUR PATH HERE\\data corpus.txt")).useDelimiter("[^a-zA-Z]+");   	
		...
		for(int i = 0; i < map.size(); i++) {
			frequency_output.add("\n" + entries.get(entries.size() - i - 1).getKey()+" "+entries.get(entries.size() - i - 1).getValue() + " time(s)");
		}
			}
		catch (IOException e) {  
			e.printStackTrace();
